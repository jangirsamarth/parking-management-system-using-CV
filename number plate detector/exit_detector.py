...
while True:
    ...
    if NumberPlateCnt is not None:
        ...
        if text and is_valid_number_plate(text):
            ...
            if result:
                in_time = time.mktime(time.strptime(result[0]))
                out_time = time.mktime(time.strptime(date_time))
                duration = out_time - in_time  # in seconds

                # Calculate price (assuming a rate of 1 rupee per minute)
                price = (duration / 60) * 1

                print(f"The time for which the car stayed = {duration} seconds")
                print(f"Price for the stay = {price} rupees")

                # Remove the car from in_time table
                delete_query = "DELETE FROM in_time WHERE car_number = %s;"
                mycursor.execute(delete_query, (car_number,))
                mydb.commit()
                print(f"Deleted old entry for car number {car_number}")
